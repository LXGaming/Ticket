...
public class BanCommand extends AbstractCommand {
    ...
    public BanCommand() {
        addAlias("ban");
        setPermission("ticket.command.ban");
        setUsage("<User>");
    }
    ...
    public void execute(Object object, List<String> arguments) {
        ...
        if (data.length() == 36) {
            ...
            if (user.isBanned()) {
                sender.sendMessage(BungeeToolbox.getTextPrefix().append(user.getName() + " is already banned").color(ChatColor.RED).create());
                return;
            }
            
            user.setBanned(true);
            MySQLQuery.updateUser(user);
            sender.sendMessage(BungeeToolbox.getTextPrefix().append(user.getName() + " banned").color(ChatColor.GREEN).create());
            return;
        }
