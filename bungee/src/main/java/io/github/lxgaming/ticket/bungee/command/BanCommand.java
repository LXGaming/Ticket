...
public class BanCommand extends AbstractCommand {
    ...
    public void execute(Object object, List<String> arguments) {
        ...
        if (BungeeToolbox.getUniqueId(sender).equals(uniqueId)) {
            sender.sendMessage(BungeeToolbox.getTextPrefix().append("You cannot ban yourself").color(ChatColor.RED).create());
            return;
        }
        ...
        if (user.isBanned()) {
            sender.sendMessage(BungeeToolbox.getTextPrefix().append(user.getName()).color(ChatColor.YELLOW).append(" has already been banned").color(ChatColor.RED).create());
            return;
        }
