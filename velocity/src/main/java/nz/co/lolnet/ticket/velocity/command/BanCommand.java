...
public class BanCommand extends AbstractCommand {
    ...
    public void execute(Object object, List<String> arguments) {
        ...
        if (data.length() == 36) {
            ...
            if (user.isBanned()) {
                source.sendMessage(VelocityToolbox.getTextPrefix().append(TextComponent.of(user.getName() + " is already banned", TextColor.RED)));
                return;
            }
            
            user.setBanned(true);
            MySQLQuery.updateUser(user);
            source.sendMessage(VelocityToolbox.getTextPrefix().append(TextComponent.of(user.getName() + " banned", TextColor.GREEN)));
            return;
        }
